    ...
    *,
    ...
    if "qwen3" in model_name_lower:
        ...
        if provider == SupportedLiteLLMProvider.Ollama:
            # Ollama's native chat API uses `think`, while several hosted Qwen
            # APIs use `enable_thinking`. Send both for local Qwen3 models so
            # the answer is returned in `content` instead of only in `thinking`.
...
class LiteLLMBase(ABC):
    ...
    def _construct_completion_args(self, history, stream: bool, tools: bool, **kwargs):
        ...
        if self.provider == SupportedLiteLLMProvider.Ollama and "qwen3" in (self.model_name or "").lower():
            ...
            if not isinstance(options, dict):
                options = {}
            options.setdefault("num_predict", int(os.environ.get("OLLAMA_RAG_NUM_PREDICT", "512")))
            options.setdefault("num_ctx", int(os.environ.get("OLLAMA_RAG_NUM_CTX", "8192")))
